...
class MediaDownloader:
    ...
    async def download_all(self, note_id: str, items: tuple[MediaItem, ...]) -> list[DownloadedMedia]:
        downloads = []
        for index, item in enumerate(items):
            downloads.append(await self.download(note_id, item, index))
        return downloads

    async def download(self, note_id: str, item: MediaItem, index: int) -> DownloadedMedia:
        self.temp_dir.mkdir(parents=True, exist_ok=True)
        path = self.temp_dir / self._filename(note_id, item, index)
        last_error: Exception | None = None
        for _ in range(self.retries + 1):
            try:
                result = self.fetcher(item.url, path)
                if asyncio.iscoroutine(result):
                    content_type, size = await result
                else:
                    content_type, size = result
                if size <= 0:
